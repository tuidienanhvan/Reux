...
export default function LoginPage() {
  ...
  useEffect(() => {
    console.log("LoginPage: GSAP loaded:", gsap);
    const root = rootRef.current;
    const wrapper = wrapperRef.current;
    const form = formRef.current;
    const links = linksRef.current;

    if (root && wrapper && form && links) {
      console.log("LoginPage: Running enter animation");
      ...
      console.error("LoginPage: rootRef, wrapperRef, formRef, or linksRef is not available");
    }
  ...
  const handleNavigate = async (path) => {
    ...
    await new Promise((resolve) => {
      if (rootRef.current && wrapperRef.current && formRef.current && linksRef.current) {
        ...
        const tl = gsap.timeline({
          onComplete: () => {
            console.log("LoginPage: Navigation exit animation completed");
        ...
        console.error("LoginPage: Exit animation skipped: refs not available");
        resolve();
      }
    });
  ...
  const onSubmit = async (data) => {
    ...
    setFormStatus(null);
    setIsExiting(true);
    setAnimationExiting(true); // Update context
    ...
    try {
      ...
      if (result.success && result.status === 200) {
        ...
        console.log("LoginPage: Starting exit animation");
        await Promise.all([
          ...
          new Promise((resolve) => {
            if (rootRef.current && wrapperRef.current && formRef.current && linksRef.current) {
              ...
              const tl = gsap.timeline({
                onComplete: () => {
                  console.log("LoginPage: Exit animation completed");
                  rootRef.current.style.overflow = "auto";
                  resolve();
                },
              });
        ...
        setAnimationExiting(false); // Reset context state
  ...
  return (
    ...
    >
      <div
        ref={wrapperRef}
        className="w-full max-w-[1280px] mx-auto flex flex-col lg:flex-row rounded-2xl bg-transparent border-0 lg:bg-[#111111] lg:border lg:border-primary/20"
      >
        <div className="hidden lg:flex md:w-[60%] items-center justify-center p-4 sm:p-6 lg:p-8">
          <img
            src={beginArtwork}
            alt="Login artwork"
        ...
        >
          <div
            className="w-full max-w-[400px] bg-base-100 rounded-xl border-2 border-primary p-4 sm:p-6 lg:p-8
                       shadow-[0_8px_16px_#E5D49E,0_4px_8px_#E5D49E] transition-all duration-500
                       hover:shadow-[0_12px_24px_#E5D49E,0_6px_12px_#E5D49E] hover:-translate-y-1
          ...
          >
            ...
            />
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-texas text-center mb-4 sm:mb-6 text-primary tracking-widest drop-shadow">
              SIGN IN
            </h1>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5" noValidate>
              <div>
                ...
                <input
                  ...
                  className="input w-full font-kurohe bg-base-200 border-2 border-primary/60 rounded-md text-sm sm:text-base
                             shadow-[inset_0_3px_6px_rgba(62,7,3,0.2)] focus:shadow-[inset_0_2px_4px_rgba(62,7,3,0.1)]
                             focus:border-primary focus:ring-2 focus:ring-primary/40 transition-all duration-300
                             placeholder:text-base-content/60 py-2 sm:py-2.5"
                  {...register("email")}
                />
                {errors.email && <p className="text-xs sm:text-sm mt-1.5 text-error">{errors.email.message}</p>}
              </div>
              <div>
                <label
                  htmlFor="password"
              ...
              <button
                ...
                className="btn w-full font-texas text-sm sm:text-base lg:text-lg tracking-wider uppercase
                           ...
                           shadow-[0_4px_8px_rgba(62,7,3,0.4)] hover:shadow-[0_6px_12px_rgba(62,7,3,0.5)]
                           hover:-translate-y-0.5 active:translate-y-0 active:shadow-[0_2px_4px_rgba(62,7,3,0.2)]
